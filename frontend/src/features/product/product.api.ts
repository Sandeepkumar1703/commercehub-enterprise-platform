...
export const productApi = {
  getProducts: async (filters: ProductQueryFilter = {}) => {
    ...
    const { data } = await axiosInstance.get<PaginatedResponse<Product>>(`/api/products?${params.toString()}`);
    return data;
  },

  searchProducts: async (keyword: string) => {
    const { data } = await axiosInstance.get<Product[]>(`/api/products/search?keyword=${encodeURIComponent(keyword)}`);
    return data;
  },
