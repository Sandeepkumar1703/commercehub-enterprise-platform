...
import { LayoutDashboard, Package, ShoppingCart, Users, Tag, MessageSquare, ArrowLeft, ShieldAlert, Store, Languages, FolderTree, CreditCard, ShieldCheck, Globe, Image } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-background text-content-primary">
      ...
      <aside className="w-60 bg-[#0F172A] text-white flex flex-col z-20 shrink-0">
        <div className="p-6 mb-2 border-b border-slate-800 flex items-center justify-between">
          <Link to={getLocalizedPath('admin/dashboard')} className="flex items-center gap-3">
        ...
        <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
          ...
          <NavLink
            to={getLocalizedPath('admin/payments')}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                isActive ? 'bg-white/10 text-white font-bold shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`
          ...
          <NavLink
            to={getLocalizedPath('admin/roles')}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                isActive ? 'bg-white/10 text-white font-bold shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`
          ...
          <NavLink
            to={getLocalizedPath('admin/languages')}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                isActive ? 'bg-white/10 text-white font-bold shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`
          ...
          <NavLink
            to={getLocalizedPath('admin/translations')}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                isActive ? 'bg-white/10 text-white font-bold shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`
          ...
          <NavLink
            to={getLocalizedPath('admin/media')}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                isActive ? 'bg-white/10 text-white font-bold shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-surface border-b border-border px-6 flex items-center justify-between shrink-0">
          ...
          <Link to={getLocalizedPath('')} className="text-xs text-brand font-semibold hover:underline flex items-center gap-1">
